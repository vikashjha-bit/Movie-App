...
function TvDetails(){
    ...
    return info?(
        <div style={{background:`linear-gradient(rgba(0,0,0,.4),rgba(0,0,0,.7),rgba(0,0,0,.9)), 
            ...
            <img className="shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)] h-[60vh] object-cover"
                 ...
                 <div className="content ml-[5%]">
                    <h1 className="text-4xl font-black text-white">
                        ...
                        <small className="text-xl ml-1 font-bold text-zinc-400">({info.detail.first_air_date.split("- ")[0]})</small>
                        </h1>
                        ...
                        <h1 className="text-3xl mt-2 mb-2">Tv Languages</h1>
            ...
            <div className="w-[100%] flex overflow-x-auto mb-5 p-5 gap-5">
            {info.detail?.seasons?.length > 0 ? (
                info.detail.seasons.map((s, i) => (
                        
                <div key={i} className="w-[25vh] mr-0">
                    <img className="min-w-[25vh] h-[30vh] object-cover rounded-lg shadow-lg" 
                        src={`https://image.tmdb.org/t/p/w500${s.poster_path || s.backdrop_path || s.profile_path}`} 
                        alt={s.name} />
                    <h1 className="text-2xl text-zinc-300 mt-3 font-semibold">{s.name}</h1>
                </div>
                ))
            ) : (
                <h1 className="text-center text-zinc-400 text-2xl">Nothing to show</h1>
            )}
            </div>
